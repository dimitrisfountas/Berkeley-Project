...
def cornersHeuristic(state, problem):
    ...
    while cornersLeftToVisit:
        ...
        min([(euclidean(curPoint, corner), corner) for corner in cornersLeftToVisit])
        cornersLeftToVisit.remove(corner)
        curPoint = corner
        totalCost += heuristic_cost
    return totalCost
def euclidean(position, problem, info={}):
    "The Euclidean distance heuristic for a PositionSearchProblem"
    xy1 = position
    xy2 = problem
    return ( (xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2 ) ** 0.5
...
def foodHeuristic(state, problem):
    ...
    while foodToEat:
        cost, food = \
            min([(euclidean(position, food), food) for food in foodToEat])
